...
class GridMappingROS:
    def __init__(self):
        rospy.init_node('grid_mapping2', anonymous=True)
        ...
        self.sensor_model_p_occ   = 0.75 #rospy.get_param('~sensor_model_p_occ', 0.75)
        self.sensor_model_p_free  = 0.45 #rospy.get_param('~sensor_model_p_free', 0.45)
        self.sensor_model_p_prior = 0.5 #rospy.get_param('~sensor_model_p_prior', 0.5)
        self.robot_frame          = '/base_link'#rospy.get_param('~robot_frame', 'base_link')
        self.map_frame            = '/map'#rospy.get_param('~map_frame', 'map')
        self.map_center_x         = -1.0 #rospy.get_param('~map_center_x', -1.0)
        self.map_center_y         = -1.0 #rospy.get_param('~map_center_y', -1.0)
        self.map_resolution       = 0.02 #rospy.get_param('~map_resolution', 0.02)
        self.map_size_x           = 4992*self.map_resolution # rospy.get_param('~map_size_x', 4992.0)
        self.map_size_y           = 4992*self.map_resolution #rospy.get_param('~map_size_y', 4992.0)
        self.map_publish_freq     = 1.0 # rospy.get_param('~map_publish_freq', 1.0)
        self.update_movement      = 0.1#rospy.get_param('~update_movement', 0.1)
        ...
        self.map_pub = rospy.Publisher('/map_topic2', OccupancyGrid, queue_size=2)
